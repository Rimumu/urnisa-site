...
import archiveData from '../data/tournament-archive.json';
...
const ArchiveTournament: React.FC = () => {
    ...
    useEffect(() => {
        // Fetch all seasons from archive
        const seasons = (archiveData as any).seasons || [];
    ...
    useEffect(() => {
        if (activeSeason.seasonId) {
            ...
            const seasonData = (archiveData.data as any)[activeSeason.seasonId] || {};
            setApiWinners(seasonData.winners || []);
            setMatches((seasonData.bracket && seasonData.bracket.matches) || []);
        }
    ...
    const fetchMyTeam = async () => {
        ...
        const data = players.find((p: any) => p.discordId === user.id);
    ...
    const fetchPlayersForSeason = async (seasonId: number) => {
        ...
        const seasonData = (archiveData.data as any)[seasonId] || {};
        setPlayersList(seasonData.players || []);
        setLoadingPlayers(false);
    };

    const fetchDuosForSeason = async (seasonId: number) => {
        const seasonData = (archiveData.data as any)[seasonId] || {};
        setDuos(seasonData.duos || []);
    };

    const fetchMyDuo = async () => {
        if (!user?.id) return;
        const seasonData = (archiveData.data as any)[activeSeason.seasonId] || {};
        const allDuos: Duo[] = seasonData.duos || [];
        const data = allDuos.find(d => d.player1DiscordId === user.id || d.player2DiscordId === user.id);
        
        if (data) {
            setMyDuo(data);
            if (data.captainDiscordId === user.id && data.team) {
                const filledTeam = [...(data.team || [])];
                while (filledTeam.length < 6) filledTeam.push(null);
                setSelectedTeam(filledTeam);
                setIsLocked(data.isLocked || false);
    ...
    const handleInitialRegister = async () => {
        alert("This tournament has ended and is in read-only mode.");
    };
      ...
      `}</style>
            ...
            <div className="relative z-20 container mx-auto px-4 pt-12">
                ...
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-start items-center mb-6 gap-4">
                    <Link to="/archive" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors font-bold tracking-wide bg-black/40 px-4 py-2 rounded-full border border-white/5 hover:border-white/20 text-sm backdrop-blur-md h-10 shrink-0">
                    ...
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide items-center">
                        {[
                            ...
                            ...(activeSeason.format.includes('Duos') ? [{ id: 'duos', label: `Duos (${duos?.length || 0})` }] : [])
                        ].map(tab => (
                            <button
                                key={tab.id}
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3 flex-wrap">
                                    ...
                                    <select
                                        ...
                                        className="bg-brand-primary/20 text-brand-primary border border-brand-primary/40 px-3 py-1 rounded-lg text-xs font-black uppercase tracking-widest cursor-pointer hover:bg-brand-primary/30 transition-all outline-none md:w-auto w-full"
                                    >
                                        {allSeasons.map(s => (
                                            <option key={s.seasonId} value={s.seasonId} className="bg-[#120507] text-white">
                                                {s.name.includes('(Archived)') ? s.name : `${s.name} (Archived)`}
                                            </option>
                                        ))}
                                    </select>
                    ...
                    <div className="min-h-[40vh] pb-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                        {activeTab === 'rules' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                {activeSeason.name.includes('Season 3') ? (
                                    <>
                                        <RuleCard title="Party Limits" icon="⚡" color="border-yellow-500/40 bg-yellow-900/10">
                                            <div className="space-y-3">
                                                ...
                                                <ul className="space-y-2 font-bold">
                                                    <li className="flex items-center gap-2"><span className="text-yellow-400">⭐</span> <span className="text-yellow-200">1 Legendary</span> allowed per party</li>
                                                    <li className="flex items-center gap-2"><span className="text-pink-400">✨</span> <span className="text-pink-200">1 Mythical</span> allowed per party</li>
                                                    <li className="flex items-center gap-2"><span className="text-purple-400">🌀</span> <span className="text-purple-200">1 Paradox</span> allowed per party</li>
                                                    <li className="flex items-center gap-2"><span className="text-blue-400">👾</span> <span className="text-blue-200">1 Ultra Beast</span> allowed per party</li>
                                                </ul>
                                            </div>
                                        </RuleCard>
                                        <RuleCard title="Gimmick Rules" icon="💥" color="border-orange-500/40 bg-orange-900/10">
                                            <div className="space-y-3">
                                        ...
                                        <RuleCard title="Banned Pokémon" icon="🚫" color="border-red-500/40 bg-red-900/10">
                                        ...
                                        <RuleCard title="Match Banning System" icon="🎯" color="border-cyan-500/40 bg-cyan-900/10">
                                            <div className="space-y-3">
                                                <div className="text-center py-2 bg-cyan-500/20 rounded-xl border border-cyan-500/30 mb-3">
                                                    <span className="text-cyan-400 font-black text-sm uppercase tracking-wider">Pre-Match Ban Phase</span>
                                                </div>
                                                <ul className="space-y-2 text-sm">
                                                    <li className="flex gap-2"><span className="text-cyan-400">1.</span><span>Players call <strong className="text-white">heads or tails</strong> (coin flip)</span></li>
                                                    <li className="flex gap-2"><span className="text-cyan-400">2.</span><span>Winner picks <strong className="text-white">ban order</strong> (first/second)</span></li>
                                                    <li className="flex gap-2"><span className="text-cyan-400">3.</span><span>Each player gets <strong className="text-yellow-400">1 ban</strong> per match</span></li>
                                                    <li className="flex gap-2"><span className="text-cyan-400">4.</span><span>Bans are made from opponent's visible roster</span></li>
                                                </ul>
                                                <p className="text-gray-400 text-xs mt-2 border-t border-white/10 pt-2">📋 Rosters visible 5-10 minutes before tournament starts after lock-ins!</p>
                                            </div>
                                        </RuleCard>
                                        ...
                                        <RuleCard title="Move & Ability Bans" icon="⛔" color="border-purple-500/40 bg-purple-900/10">
                                            <div className="space-y-3">
                            ...
                            <div className="relative z-10 bg-black/40 backdrop-blur-xl rounded-[3rem] border border-white/10 p-8 shadow-2xl animate-in fade-in slide-in-from-left-4 duration-500">
                                ...
                                <div className="space-y-6">
                                    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                                        <h3 className="text-4xl font-black text-purple-400 uppercase tracking-tighter">👥 Duos Teams</h3>
                                    ...
                                    {duos.length === 0 ? (
                                        <div className="text-center py-24 text-gray-600 font-bold italic">No duos paired yet!</div>
                                    ) : (
