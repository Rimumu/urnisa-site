...
import InventoryModal from '../components/InventoryModal'; // Import InventoryModal
...
const Minecraft: React.FC = () => {
  ...
  const handleLinkSubmit = async (e: React.FormEvent) => {
      ...
      setLinkStatus('idle');
      if (!/^[a-zA-Z0-9_]{3,16}$/.test(mcInput)) { setLinkStatus('error'); return; }
      try {
          const response = await fetch(`${DISCORD_API_URL}/api/minecraft/link`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ discordId: user.id, discordUsername: user.username, discordAvatar: user.avatar, minecraftUsername: mcInput })
          });
          if (response.ok) {
              ...
              setTimeout(() => { setShowLinkModal(false); setLinkStatus('idle'); }, 1500);
          } else if (response.status === 409) { setLinkStatus('conflict'); } else { setLinkStatus('error'); }
      } catch (e) { setLinkStatus('error'); }
  };
  ...
  const handleUnlinkMinecraft = async () => {
      ...
      try {
          ...
          if (response.ok) {
              ...
              localStorage.setItem('urnisa_mc_user', JSON.stringify(updatedUser));
              setShowUnlinkModal(false); setMenuOpen(false);
          }
      } catch (e) {}
  };
  ...
  const handleApplyWhitelist = async () => {
      ...
      if (!user.minecraftUsername) { setShowMCAlert(true); return; }
      setApplying(true); setWhitelistStatus(null);
      try {
          ...
          const data = await response.json();
          if (response.ok) { setWhitelistStatus({ type: 'success', msg: data.message || "Application Sent!" }); }
          else if (response.status === 403) { setShowSubAlert(true); }
          else if (response.status === 409) { setWhitelistStatus({ type: 'success', msg: "Already pending!" }); }
          else { setWhitelistStatus({ type: 'error', msg: data.error || "Failed" }); }
      } catch (e) { setWhitelistStatus({ type: 'error', msg: "Server Error" }); }
      finally { setApplying(false); setTimeout(() => setWhitelistStatus(null), 5000); }
  };

  const handleCopy = () => { navigator.clipboard.writeText(SERVER_IP); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  ...
  return (
    <div className="min-h-screen py-8 font-sans text-white relative">
        ...
        {/* Inventory Modal */}
        {showInventory && user && (
            <InventoryModal discordId={user.id} onClose={() => setShowInventory(false)} />
        )}

        {/* ... (Alert Modals: Whitelist Status, LoginAlert, MCAlert, SubAlert - same as before) ... */}
        {whitelistStatus && (
        ...
        {showLoginAlert && (
            <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
                <div className="bg-[#1a0b0e] border border-white/10 p-6 rounded-3xl w-full max-w-sm shadow-2xl text-center relative">
                    ...
                    <div className="w-16 h-16 bg-red-900/20 rounded-full flex items-center justify-center mx-auto mb-4 text-red-500 text-3xl border border-red-500/20">🔒</div>
                    ...
                    <button onClick={() => setShowLoginAlert(false)} className="w-full bg-white/10 hover:bg-white/20 text-white font-bold py-3 rounded-xl transition-colors">Close</button>
                </div>
            </div>
        )}
        {showMCAlert && (
            <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
                <div className="bg-[#1a0b0e] border border-white/10 p-6 rounded-3xl w-full max-w-sm shadow-2xl text-center relative">
                    ...
                    <div className="w-16 h-16 bg-amber-900/20 rounded-full flex items-center justify-center mx-auto mb-4 text-amber-500 text-3xl border border-amber-500/20">🔗</div>
                    ...
                    <button onClick={() => setShowMCAlert(false)} className="w-full bg-white/10 hover:bg-white/20 text-white font-bold py-3 rounded-xl transition-colors">Close</button>
                </div>
            </div>
        )}
        {showSubAlert && (
            <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
                <div className="bg-[#1a0b0e] border border-white/10 p-6 rounded-3xl w-full max-w-sm shadow-2xl text-center relative">
                    ...
                    <div className="w-16 h-16 bg-purple-900/20 rounded-full flex items-center justify-center mx-auto mb-4 text-purple-400 text-3xl border border-purple-500/20">⭐</div>
                    ...
                    <button onClick={() => setShowSubAlert(false)} className="w-full bg-white/10 hover:bg-white/20 text-white font-bold py-3 rounded-xl transition-colors">Close</button>
                </div>
            </div>
        )}
        ...
        <div className="absolute top-0 right-0 p-4 z-50 flex justify-end w-full">
            {loading ? (
                ...
                <div className="relative">
                    ...
                    {menuOpen && (
                        <div className="absolute right-0 mt-2 w-64 bg-[#1e1f22] rounded-xl shadow-xl border border-white/10 overflow-hidden animate-in fade-in slide-in-from-top-2 z-50">
                            ...
                            <button 
                                onClick={() => { setShowInventory(true); setMenuOpen(false); }}
                                className="w-full text-left px-4 py-3 text-sm text-brand-accent hover:bg-brand-accent/10 transition-colors flex items-center gap-2 font-bold border-b border-white/5"
                            >
                                <BackpackIcon />
                                My Inventory
                            </button>
                            ...
                            {user.minecraftUsername ? (
                                <div className="px-4 py-3 border-b border-white/5">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button onClick={() => { setShowUnlinkModal(true); setMenuOpen(false); }} className="text-[10px] text-red-400 hover:text-red-300 underline">Unlink</button>
                                    </div>
                                </div>
                            ) : (
                                <button onClick={() => { setShowLinkModal(true); setMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-brand-primary hover:bg-brand-primary/10 transition-colors flex items-center gap-2 font-bold">
                                    <span>🔗</span> Link Minecraft Account
                                </button>
                            )}

                            <button onClick={logout} className="w-full text-left px-4 py-3 text-sm text-gray-400 hover:text-white hover:bg-white/5 transition-colors flex items-center gap-2">
                                <PowerIcon />
                ...
                <button onClick={loginRedirect} className="flex items-center gap-2 bg-[#5865F2] hover:bg-[#4752c4] text-white font-bold py-2 px-6 rounded-full shadow-lg transition-all hover:scale-105">
                    <DiscordLogo />
                    Login with Discord
                </button>
            )}
        ...
        {/* ... (Link Modal and Unlink Modal - same as before) ... */}
        {showLinkModal && (
            <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="px-8 pt-12 pb-8 text-center">
                        ...
                        <form onSubmit={handleLinkSubmit} className="space-y-6 text-left">
                            <div>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <img src={mcInput ? `https://mc-heads.net/avatar/${mcInput}/20` : "https://mc-heads.net/avatar/Steve/20"} alt="" className="w-5 h-5 rounded-sm grayscale opacity-70 transition-all duration-300" onError={(e) => (e.currentTarget.style.display = 'none')} />
                                    </div>
                                    <input type="text" value={mcInput} onChange={(e) => { setMcInput(e.target.value); setLinkStatus('idle'); }} className={`w-full bg-black/40 border rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none font-mono transition-all placeholder:text-gray-600 ${linkStatus === 'conflict' ? 'border-red-500 focus:border-red-500 focus:ring-1 focus:ring-red-500' : 'border-white/10 focus:border-brand-primary focus:ring-1 focus:ring-brand-primary'}`} placeholder="Notch" required autoFocus />
                                </div>
                            ...
                            <div className="flex gap-3">
                                <button type="button" onClick={() => setShowLinkModal(false)} className="flex-1 bg-white/5 hover:bg-white/10 text-gray-300 font-bold py-3 rounded-xl transition-colors text-sm">Cancel</button>
                                <button type="submit" disabled={linkStatus === 'success' || linkStatus === 'conflict'} className={`flex-1 font-bold py-3 rounded-xl transition-all shadow-lg text-sm text-white ${linkStatus === 'success' ? 'bg-green-600 scale-105' : linkStatus === 'conflict' ? 'bg-red-600 cursor-not-allowed' : 'bg-green-600 hover:bg-green-500 hover:scale-[1.02]'}`}>{linkStatus === 'success' ? '✓ Linked!' : linkStatus === 'conflict' ? 'Taken ✕' : 'Link'}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
                <div className="bg-[#1a0b0e] border border-white/10 p-0 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden relative">
                    ...
                    <div className="px-8 pt-12 pb-8 text-center">
                        ...
                        <p className="text-gray-400 text-sm mb-6">Are you sure you want to unlink <strong>{user?.minecraftUsername}</strong>? You will lose your whitelist status on the server.</p>
                        <div className="flex gap-3">
                            <button onClick={() => setShowUnlinkModal(false)} className="flex-1 bg-white/5 hover:bg-white/10 text-gray-300 font-bold py-3 rounded-xl transition-colors text-sm">Cancel</button>
                            <button onClick={handleUnlinkMinecraft} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg text-sm">Unlink</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        {/* ... (Hero Section and Content Grid - same as before, but Gacha button updated to link correctly) ... */}
        <div className="flex flex-col items-center text-center space-y-6 mb-16 animate-in fade-in slide-in-from-bottom-4 duration-700 pt-16">
            <div className="relative z-10">
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight leading-tight drop-shadow-2xl">
                    WELCOME TO <span className="text-brand-primary">URNISA</span> MINECRAFT<br /><span className="text-[#60a5fa] drop-shadow-[0_0_10px_rgba(96,165,250,0.5)]">COBBLEMON</span> SERVER
                </h1>
            ...
            <p className="text-gray-300 max-w-2xl text-lg md:text-xl px-4">Embark on a journey with nisa and use your balls with her to catch some little gremlins. Well... some are big.</p>
        ...
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-6 h-full">
                <div className="block flex-1 bg-black/20 backdrop-blur-sm border border-white/5 p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group cursor-not-allowed opacity-70 grayscale">
                    <div className="absolute inset-0 bg-black/60 z-20 flex items-center justify-center"><div className="bg-brand-primary text-white font-black text-xl uppercase px-6 py-2 rounded-full transform -rotate-12 border-2 border-white shadow-xl">Coming Soon</div></div>
                    ...
                    <div className="flex items-center gap-4 mb-4 opacity-50"><div className="w-12 h-12 rounded-2xl bg-[#60a5fa]/20 flex items-center justify-center text-2xl shadow-inner">🎫</div><h2 className="text-2xl font-black text-white uppercase tracking-wide">Bingo Card</h2></div>
                    <p className="text-gray-500 font-medium leading-relaxed">Complete your card by catching Pokémon, building structures, and participating in events to win amazing prizes!</p>
                    <div className="mt-6 flex items-center text-gray-600 font-bold text-sm uppercase tracking-wider">View Card <span className="ml-2">→</span></div>
                </div>

                {/* Gacha Pack Box (Now Active) */}
                <Link to="/minecraft/gacha" className="block flex-1 bg-black/40 backdrop-blur-md border border-white/10 p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group hover:border-brand-accent/50 transition-colors">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-brand-accent/10 rounded-full blur-2xl pointer-events-none -translate-y-1/2 translate-x-1/2 group-hover:bg-brand-accent/20 transition-colors"></div>
                    <div className="flex items-center gap-4 mb-4">
                        <div className="w-12 h-12 rounded-2xl bg-brand-accent/20 flex items-center justify-center text-2xl shadow-inner group-hover:scale-110 transition-transform">🥩</div>
                        <h2 className="text-2xl font-black text-white uppercase tracking-wide">Gacha Pack</h2>
                    </div>
                    <p className="text-gray-300 font-medium leading-relaxed">Slice up some lamb chop and wagyu packs and see if you will get flavorful rewards!</p>
                    <div className="mt-6 flex items-center text-brand-accent font-bold text-sm uppercase tracking-wider group-hover:underline">Open Packs <span className="ml-2">→</span></div>
                </Link>
            </div>

            <div className="bg-black/30 backdrop-blur-lg border border-white/10 p-8 rounded-[2.5rem] shadow-xl flex flex-col h-full">
                <div className="flex items-center gap-4 mb-6 border-b border-white/5 pb-4"><div className="w-12 h-12 rounded-2xl bg-brand-primary/20 flex items-center justify-center text-2xl">🎮</div><h2 className="text-2xl font-bold text-white">How to Join</h2></div>
                <div className="space-y-6 relative">
                    <div className="absolute left-[19px] top-8 bottom-8 w-0.5 bg-white/10 -z-10"></div>
                    {[{ step: 1, title: "Download Modpack", desc: "Get the Cobblemon Academy modpack below." }, { step: 2, title: "Get Whitelisted", desc: "Apply below to get whitelisted to the server." }, { step: 3, title: "Launch Game", desc: "Launch Minecraft with the modpack and go to Multiplayer." }, { step: 4, title: "Connect", desc: `Add server IP: ${SERVER_IP}` }].map((item) => (
                        <div key={item.step} className="flex gap-6 items-start">
                            <div className="w-10 h-10 rounded-full bg-[#1f090c] border-2 border-brand-primary text-brand-primary font-black flex items-center justify-center shrink-0 z-10 shadow-lg">{item.step}</div>
                            <div className="pt-1"><div className="font-bold text-white text-lg">{item.title}</div><div className="text-gray-400 text-sm">{item.desc}</div></div>
                        </div>
                    ))}
                </div>
                <div className="mt-auto pt-8 flex flex-col gap-3">
                    <a href="https://www.curseforge.com/minecraft/modpacks/cobblemon-academy" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full text-white font-bold py-4 rounded-xl transition-all shadow-lg bg-[#f16436] hover:bg-[#d6552a] hover:scale-[1.02] shadow-orange-500/20"><CurseforgeLogo />Download Modpack</a>
                    <button onClick={handleApplyWhitelist} disabled={applying} className={`flex items-center justify-center gap-2 w-full text-white font-bold py-4 rounded-xl transition-all shadow-lg ${applying ? 'bg-gray-600 cursor-wait' : 'bg-[#5865F2] hover:bg-[#4752c4] hover:scale-[1.02] shadow-indigo-500/20'}`}>
                        {applying ? <>Verifying Status...</> : <><svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"></path><rect x="9" y="3" width="6" height="4" rx="2" ry="2"></rect><path d="M9 14l2 2 4-4"></path></svg>Apply for Whitelist!</>}
                    </button>
                </div>
            </div>
        </div>
