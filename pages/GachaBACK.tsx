...
const GachaBACK: React.FC = () => {
    ...
    const [keys, setKeys] = useState({ lambKeys: 0, steakKeys: 0 });
        ...
        if (user?.id) {
            fetch(`${DISCORD_API_URL}/api/packs?discordId=${user.id}`)
                ...
                .then(data => {
                    if (data && !data.error) {
                        setKeys({ 
                            lambKeys: data.lambKeys ?? data.lambPacks ?? 0, 
                            steakKeys: data.steakKeys ?? data.wagyuPacks ?? 0 
                        });
                    }
                })
                .catch(console.error);
    ...
    const handleOpenCrate = async (type: CrateType) => {
        ...
        // Optimistic check
        if ((type === 'lamb' && keys.lambKeys < 1) || (type === 'steak' && keys.steakKeys < 1)) {
        ...
        try {
            // Deduct key
            // Note: backend expects 'type', and might still be expecting 'wagyu' internally if we didn't patch perfectly, but we patched it to 'steak'
            ...
            if (res.ok && data.success) {
                if (type === 'lamb') setKeys(k => ({ ...k, lambKeys: data.remaining }));
                else setKeys(k => ({ ...k, steakKeys: data.remaining }));
                ...
                alert(data.error || "Failed to use key");
            }
        } catch (e) {
