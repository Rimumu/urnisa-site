...
const InventoryCardImage: React.FC<{ item: InventoryItem }> = ({ item }) => {
    ...
    // Check if this item is a hat
    const isHat = availableHats.some((hat: any) => hat.id === item.itemId);
    ...
    useEffect(() => {
        const verifyImage = async () => {
            ...
            // Check if it's a TCG pack from the shop
            const shopPack = TCG_PACKS.find(p => p.id === item.itemId || item.name.toLowerCase().includes(p.name.toLowerCase()));
            if (shopPack?.coverImage) {
                setImgSrc(shopPack.coverImage);
                return;
            }

            // Check if it's a Gacha item
            const gachaItem = LAMB_POOL.find(p => p.id.toString() === item.itemId) || WAGYU_POOL.find(p => p.id.toString() === item.itemId);
            if (gachaItem?.image) {
                setImgSrc(gachaItem.image);
                return;
            }
    ...
    if (isHat) {
        return (
            <div className="w-full h-full flex items-center justify-center -translate-y-2">
    ...
    const tcgPack = TCG_PACKS.find(p => p.id === item.itemId || item.name.toLowerCase().includes(p.name.toLowerCase()));
    const isGenericPack = item.name.toLowerCase().includes("tcg") && item.name.toLowerCase().includes("pack");
    const isTCG = !!tcgPack || isGenericPack;

    if (isTCG) {
        return (
            <OptimizedImage 
                src={imgSrc || (tcgPack ? tcgPack.coverImage : '')} 
                alt={item.name} 
                className="w-full h-full drop-shadow-lg transition-transform duration-300 group-hover:scale-110 rounded-lg"
                contain={false}
                pixelated={false}
                imgStyle={tcgPack && tcgPack.cropScale ? {
                    transform: `scale(${tcgPack.cropScale})`,
                    objectPosition: tcgPack.cropPosition || 'center'
                } : undefined}
    ...
    return (
        <OptimizedImage 
            src={imgSrc} 
            alt={item.name} 
            className="w-full h-full drop-shadow-lg transition-transform duration-300 group-hover:scale-110 object-contain rounded-lg"
            contain={true}
            pixelated={imgSrc.includes('cobblemon.tools') || imgSrc.includes('PokeAPI') || !imgSrc.includes('assets/images/')}
            onError={handleImageError}
        />
    );
};

const InventoryBACK: React.FC = () => {
    ...
    const [filter, setFilter] = useState<'All' | 'Unclaimed' | 'Claimed' | 'Loot' | 'Cosmetic' | 'TCG' | 'Currency'>('All');
    ...
    const getItemCategory = (item: InventoryItem): 'Loot' | 'Cosmetic' | 'TCG' | 'Currency' => {
        // 1. Check if it's a Minecraft hat or hat bag
        const isHat = availableHats.some((hat: any) => hat.id === item.itemId);
        if (isHat || item.name.toLowerCase().includes('hat bag') || item.name.toLowerCase().includes('hat') || item.name.toLowerCase().includes('cosmetic')) {
            return 'Cosmetic';
        }

        // 2. Check if it's a TCG pack or TCG card
        const isShopPack = TCG_PACKS.some(p => p.id === item.itemId);
        if (isShopPack || item.name.toLowerCase().includes('tcg') || item.name.toLowerCase().includes('pack') || item.name.toLowerCase().includes('card')) {
            return 'TCG';
        }

        // 3. Check if it's in LAMB_POOL or WAGYU_POOL
        const poolItem = LAMB_POOL.find(p => p.id.toString() === item.itemId) || WAGYU_POOL.find(p => p.id.toString() === item.itemId);
        if (poolItem) {
            if (poolItem.subType === 'TCG') return 'TCG';
            if (poolItem.subType === 'Loot') return 'Loot';
            if (poolItem.subType === 'Cosmetic') return 'Cosmetic';
            if (poolItem.subType === 'Currency') return 'Currency';
        }

        // 4. Default rules based on name keywords
        const nameLower = item.name.toLowerCase();
        if (nameLower.includes('loot ball') || nameLower.includes('lootball') || nameLower.includes('crate') || nameLower.includes('key')) {
            return 'Loot';
        }
        if (nameLower.includes('coin') || nameLower.includes('dollar') || nameLower.includes('nisaball') || nameLower.includes('$') || nameLower.includes('currency')) {
            return 'Currency';
        }

        return 'Loot'; // fallback
    };
    ...
    const getDisplayRarity = (item: InventoryItem) => {
        const isHat = availableHats.some((hat: any) => hat.id === item.itemId);
        if (isHat) return 'Legendary';
        const shopPack = TCG_PACKS.find(p => p.id === item.itemId);
        if (shopPack) return 'Legendary';
        const gachaItem = LAMB_POOL.find(p => p.id.toString() === item.itemId) || WAGYU_POOL.find(p => p.id.toString() === item.itemId);
        if (gachaItem && gachaItem.rarity) return gachaItem.rarity;
        return item.rarity;
    };
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
                    ...
                    <div className="flex bg-black/40 rounded-xl p-1 border border-white/10 backdrop-blur-sm overflow-x-auto w-full md:w-auto">
                        {(['All', 'Unclaimed', 'Claimed', 'Loot', 'Cosmetic', 'TCG', 'Currency'] as const).map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap flex-1 md:flex-none ${filter === f ? 'bg-white/10 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                {f}
                            </button>
                        ))}
                            ...
                            {displayedItems.map(item => (
                                <div 
                                    ...
                                    className={`
                                        ...
                                        ${item.claimed ? 'border-gray-800 bg-black/20 opacity-50 grayscale' : getRarityStyles(getDisplayRarity(item))}
                                        ${!item.claimed && 'hover:-translate-y-1 hover:shadow-xl'}
                                    `}
                                >
                                    {/* Top Badge */}
                                    <div className="absolute top-2 right-2 z-20">
                                        <span className={`text-[9px] uppercase font-black tracking-wider px-2 py-0.5 rounded-full shadow-sm backdrop-blur-md ${getRarityBadge(getDisplayRarity(item))}`}>
                                    ...
                                    <div className="relative w-full aspect-square overflow-hidden flex-shrink-0">
                                        ...
                                        {(getDisplayRarity(item) === 'Mythical' || getDisplayRarity(item) === 'Legendary') && (
